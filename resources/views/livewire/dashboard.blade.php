    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-5">
        ...
        <div class="boron-card rounded-xl sm:rounded-lg">
            <div class="boron-card-body p-5 md:p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="mb-1 text-[11px] md:text-sm font-semibold text-[#8a969c] uppercase">Pemesanan / Registrasi</p>
                        <h3 class="text-2xl md:text-3xl font-bold text-[#313a46] dark:text-white">{{ $stats['pendaftar']['total'] }}</h3>
                    </div>
                    <span class="flex size-10 md:size-12 items-center justify-center rounded-full bg-[#60addf]/20 shrink-0">
                        <i class="ti ti-users-plus text-lg md:text-xl text-[#60addf]"></i>
                    </span>
                </div>
                <div class="mt-4 flex items-center gap-1.5 border-t border-dashed border-[#e7e9eb] dark:border-[#37394d] pt-3">
                    <i class="ti {{ $stats['pendaftar']['change']['up'] ? 'ti-trending-up text-[#70bb63]' : 'ti-trending-down text-[#ed6060]' }} text-sm md:text-base"></i>
                    <span class="text-xs md:text-sm font-bold {{ $stats['pendaftar']['change']['up'] ? 'text-[#70bb63]' : 'text-[#ed6060]' }}">{{ $stats['pendaftar']['change']['val'] }}%</span>
                    <span class="text-[10px] md:text-xs text-[#8a969c] ml-1">{{ $comparisonLabel }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="boron-card rounded-xl sm:rounded-lg">
            <div class="boron-card-body p-5 md:p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="mb-1 text-[11px] md:text-sm font-semibold text-[#8a969c] uppercase">{{ $filterMonth === 'all' ? 'Aktif Tahun Ini' : 'Aktif Bulan Ini' }}</p>
                        <h3 class="text-2xl md:text-3xl font-bold text-[#313a46] dark:text-white">{{ $stats['aktif']['total'] }}</h3>
                    </div>
                    <span class="flex size-10 md:size-12 items-center justify-center rounded-full bg-[#70bb63]/20 shrink-0">
                        <i class="ti ti-wifi text-lg md:text-xl text-[#70bb63]"></i>
                    </span>
                </div>
                <div class="mt-4 flex items-center gap-1.5 border-t border-dashed border-[#e7e9eb] dark:border-[#37394d] pt-3">
                    <i class="ti {{ $stats['aktif']['change']['up'] ? 'ti-trending-up text-[#70bb63]' : 'ti-trending-down text-[#ed6060]' }} text-sm md:text-base"></i>
                    <span class="text-xs md:text-sm font-bold {{ $stats['aktif']['change']['up'] ? 'text-[#70bb63]' : 'text-[#ed6060]' }}">{{ $stats['aktif']['change']['val'] }}%</span>
                    <span class="text-[10px] md:text-xs text-[#8a969c] ml-1">{{ $comparisonLabel }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="boron-card rounded-xl sm:rounded-lg">
            <div class="boron-card-body p-5 md:p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="mb-1 text-[11px] md:text-sm font-semibold text-[#8a969c] uppercase">Belum Aktif (Proses)</p>
                        <h3 class="text-2xl md:text-3xl font-bold text-[#313a46] dark:text-white">{{ $stats['proses']['total'] }}</h3>
                    </div>
                    <span class="flex size-10 md:size-12 items-center justify-center rounded-full bg-[#ebb751]/20 shrink-0">
                        <i class="ti ti-loader text-lg md:text-xl text-[#ebb751] animate-spin-slow"></i>
                    </span>
                </div>
                <div class="mt-4 flex items-center gap-1.5 border-t border-dashed border-[#e7e9eb] dark:border-[#37394d] pt-3">
                    <i class="ti {{ $stats['proses']['change']['up'] ? 'ti-trending-up text-[#ed6060]' : 'ti-trending-down text-[#70bb63]' }} text-sm md:text-base"></i>
                    <span class="text-xs md:text-sm font-bold {{ $stats['proses']['change']['up'] ? 'text-[#ed6060]' : 'text-[#70bb63]' }}">{{ $stats['proses']['change']['val'] }}%</span>
                    <span class="text-[10px] md:text-xs text-[#8a969c] ml-1">{{ $comparisonLabel }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="boron-card bg-[#1e5d87] border-[#1e5d87] text-white rounded-xl sm:rounded-lg overflow-hidden relative">
            ...
            <div class="boron-card-body p-5 md:p-6 relative z-10">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="mb-1 text-[11px] md:text-sm font-semibold text-white/70 uppercase">Keseluruhan Aktif</p>
                        <h3 class="text-2xl md:text-3xl font-black text-white">{{ $stats['total_all']['total'] }}</h3>
                    </div>
                    <span class="flex size-10 md:size-12 items-center justify-center rounded-full bg-white/20 shrink-0">
                        <i class="ti ti-server text-lg md:text-xl text-white"></i>
                    </span>
                </div>
                <div class="mt-4 flex items-center gap-1.5 border-t border-dashed border-white/20 pt-3">
                    <span class="text-[10px] md:text-xs text-white/80 flex items-center"><i class="ti ti-circle-check text-[#70bb63] mr-1 text-sm"></i> Data real-time seluruh sistem</span>
                </div>
            </div>
        </div>
    </div>
