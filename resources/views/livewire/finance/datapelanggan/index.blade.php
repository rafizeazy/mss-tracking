                                ...
                                <td class="md:px-6 md:py-4 md:text-center mt-3 md:mt-0 block md:table-cell">
                                    <div class="flex flex-col md:flex-row items-center justify-center gap-2 md:gap-1.5 md:flex-wrap">
                                        ...
                                        <div class="grid grid-cols-2 md:flex w-full md:w-auto gap-2 md:gap-1.5">
                                            <a href="{{ route('form.formulir', $cust->id) }}" target="_blank" class="flex justify-center btn-boron btn-boron-outline-secondary !py-2 md:!px-2 md:!py-1 text-xs md:text-[11px] rounded-lg md:rounded" title="Cetak Formulir">
                                                <i class="ti ti-file-text text-[#1e5d87] mr-1"></i> FORM
                                            </a>
                                            ...
                                            <a href="{{ route('customer.invoice', $cust->id) }}" target="_blank" class="flex justify-center btn-boron btn-boron-outline-warning !py-2 md:!px-2 md:!py-1 text-xs md:text-[11px] !text-[#ebb751] !border-[#ebb751] rounded-lg md:rounded" title="Lihat Invoice">
                                                <i class="ti ti-file-invoice mr-1"></i> INV
                                            </a>
                                        </div>
                                    </div>
                                </td>
    ...
    @if($showModal && $selectedCustomer)
        <div class="fixed inset-0 z-[999] flex items-end md:items-center justify-center bg-black/50 backdrop-blur-sm p-0 md:p-4" wire:transition.opacity>
            <div class="bg-white dark:bg-[#1e1f27] rounded-t-2xl md:rounded-xl shadow-2xl w-full max-w-5xl h-[90vh] md:h-auto md:max-h-[90vh] flex flex-col overflow-hidden border-t md:border border-[#e7e9eb] dark:border-[#37394d]" @click.stop>
                ...
                <div class="px-5 md:px-6 py-4 md:py-4 border-b border-[#e7e9eb] dark:border-[#37394d] flex flex-col sm:flex-row sm:justify-between items-start sm:items-center gap-4 sm:gap-0 bg-[#f8f9fa] dark:bg-[#15151b]">
                    <div class="w-full sm:w-auto flex justify-between items-center">
                        ...
                        <button wire:click="closeModal" class="sm:hidden text-[#8a969c] bg-gray-200/50 p-1.5 rounded-full hover:text-[#ed6060] transition-colors"><i class="ti ti-x text-xl"></i></button>
                    </div>
                    
                    <div class="w-full sm:w-auto flex items-center justify-between sm:justify-start gap-3">
                        <a href="{{ route('form.formulir', $selectedCustomer->id) }}" target="_blank" class="w-full sm:w-auto justify-center btn-boron !py-2 sm:!py-1.5 flex items-center gap-2 sm:gap-1 bg-[#1e5d87]/10 text-[#1e5d87] hover:bg-[#1e5d87]/20 border border-[#1e5d87]/20 transition-colors font-medium text-sm sm:text-xs dark:bg-[#60addf]/10 dark:text-[#60addf] dark:border-[#60addf]/20 rounded-lg">
                            <i class="ti ti-file-text text-lg sm:text-base"></i> Cetak Formulir
                        </a>
                        <button wire:click="closeModal" class="hidden sm:block text-[#8a969c] hover:text-[#ed6060] transition-colors"><i class="ti ti-x text-2xl"></i></button>
                    </div>
                </div>
                ...
                <div class="p-5 md:p-6 overflow-y-auto boron-scrollbar flex-1 text-[13px] md:text-sm text-[#4c4c5c] dark:text-[#aab8c5]">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                        ...
                        <div class="space-y-6 md:space-y-8">
                            <div class="space-y-3">
                                <h5 class="font-bold text-[#ebb751] uppercase text-xs border-b border-dashed border-[#e7e9eb] dark:border-[#37394d] pb-2 mb-3"><i class="ti ti-report-money text-sm md:text-base mr-1"></i> Layanan & Komersial</h5>
                                ...
                                <div class="grid grid-cols-[100px_1fr] md:grid-cols-3 gap-2"><span class="text-[#8a969c]">B. Bulanan</span><span class="md:col-span-2 font-bold text-[#70bb63]">: Rp {{ number_format($selectedCustomer->monthly_fee ?? 0, 0, ',', '.') }}</span></div>
                                <div class="grid grid-cols-[100px_1fr] md:grid-cols-3 gap-2"><span class="text-[#8a969c]">B. Registrasi</span><span class="md:col-span-2 font-medium text-[#313a46] dark:text-white">: Rp {{ number_format($selectedCustomer->registration_fee ?? 0, 0, ',', '.') }}</span></div>
                                <div class="grid grid-cols-[100px_1fr] md:grid-cols-3 gap-2 mt-2 pt-2 border-t border-[#e7e9eb] dark:border-[#37394d]"><span class="text-[#8a969c]">Marketing</span><span class="md:col-span-2 font-medium text-[#313a46] dark:text-white">: {{ $selectedCustomer->marketing_name ?? '-' }}</span></div>
                            </div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    <p class="text-[11px] font-semibold text-[#8a969c] uppercase mb-1">PIC Keuangan (Finance)</p>
                                    <div class="bg-[#f8f9fa] dark:bg-white/5 p-3 md:p-4 rounded-lg text-xs md:text-sm space-y-2 border border-[#e7e9eb] dark:border-[#37394d]">
                                        <p><span class="text-[#8a969c] w-[80px] md:w-[90px] inline-block">Nama:</span> <span class="font-medium text-[#313a46] dark:text-white">{{ $selectedCustomer->finance_name ?? '-' }}</span></p>
                                        <p><span class="text-[#8a969c] w-[80px] md:w-[90px] inline-block">No. HP:</span> <span class="font-medium text-[#313a46] dark:text-white">{{ $selectedCustomer->finance_phone ?? '-' }}</span></p>
                                        <p><span class="text-[#8a969c] w-[80px] md:w-[90px] inline-block align-top">Email:</span> <span class="font-medium text-[#313a46] dark:text-white inline-block w-[calc(100%-80px)] md:w-[calc(100%-90px)] break-words">{{ $selectedCustomer->finance_email ?? '-' }}</span></p>
                                        <p><span class="text-[#8a969c] w-[80px] md:w-[90px] inline-block align-top mt-1">Alamat Tagihan:</span> <span class="font-medium text-[#313a46] dark:text-white inline-block w-[calc(100%-80px)] md:w-[calc(100%-90px)] mt-1">{{ $selectedCustomer->billing_address ?? '-' }}</span></p>
                                    </div>
                                </div>
                    ...
                    <div class="mt-8 mb-4 md:mb-0">
                        ...
                        <div class="grid grid-cols-2 md:flex flex-wrap gap-2 md:gap-3">
                            ...
                            @if($selectedCustomer->payment_proof_file_path) <a href="{{ asset('storage/' . $selectedCustomer->payment_proof_file_path) }}" target="_blank" class="flex justify-center px-3 py-2 md:py-1.5 bg-[#70bb63]/10 text-[#70bb63] rounded-lg md:rounded text-xs font-semibold hover:bg-[#70bb63] hover:text-white transition-colors"><i class="ti ti-receipt mr-1 text-sm md:text-xs"></i> Bukti Bayar</a> @endif
                        </div>
                    </div>
