                        ...
                        @forelse ($services as $srv)
                            <tr class="flex flex-col md:table-row border-b border-[#e7e9eb] md:border-none dark:border-[#37394d] p-5 md:p-0 gap-3 md:gap-0 hover:bg-[#f8f9fa] dark:hover:bg-white/5 transition-colors">
                                ...
                                <td class="flex justify-between items-center md:table-cell md:px-6 md:py-4 whitespace-nowrap border-b border-dashed border-[#e7e9eb] md:border-none dark:border-[#37394d] pb-3 md:pb-0">
                                    ...
                                    <span class="font-bold md:font-medium {{ $srv->spk && \Carbon\Carbon::parse($srv->spk->due_date)->isPast() ? 'text-[#ed6060]' : 'text-[#ebb751]' }}">
                                        @if($srv->spk && $srv->spk->due_date)
                                            {{ \Carbon\Carbon::parse($srv->spk->due_date)->format('d M Y') }}
                                        @else
                                            -
                                        @endif
                                    </span>
                                ...
                                <td class="flex justify-between items-start md:items-center md:table-cell md:px-6 md:py-4 border-b border-dashed border-[#e7e9eb] md:border-none dark:border-[#37394d] pb-3 md:pb-0">
                                    ...
                                    <div class="text-right md:text-left w-[60%] md:w-auto">
                                        <p class="font-bold md:font-medium text-[#313a46] dark:text-white line-clamp-1">{{ $srv->customer->company_name }}</p>
                                        <p class="text-[11px] md:text-xs text-[#8a969c] mt-0.5 line-clamp-2 md:line-clamp-none">{{ $srv->installation_address ?? $srv->customer->installation_address ?? $srv->customer->company_address }}</p>
                                    </div>
                                </td>
                                ...
                                <td class="flex justify-between items-start md:items-center md:table-cell md:px-6 md:py-4 whitespace-nowrap border-b border-dashed border-[#e7e9eb] md:border-none dark:border-[#37394d] pb-3 md:pb-0">
                                    ...
                                    <div class="text-right md:text-left">
                                        <p class="font-bold md:font-medium text-[#313a46] dark:text-white">{{ $srv->bandwidth ?? '-' }}</p>
                                        <p class="text-[11px] md:text-xs text-[#8a969c] mt-0.5">{{ $srv->service_type ?? '-' }}</p>
                                    </div>
                                </td>
                                ...
                                <td class="md:px-6 md:py-4 md:text-center mt-3 md:mt-0 block md:table-cell">
                                    <a href="{{ route('noc.tracking.show', $srv->id) }}" wire:navigate class="w-full md:w-auto inline-flex justify-center items-center btn-boron btn-boron-outline-primary !px-4 !py-2.5 md:!px-3 md:!py-1 text-sm md:text-xs rounded-lg md:rounded">
